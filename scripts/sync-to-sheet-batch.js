...
/**
 * @fileoverview Syncs government opportunity data to Google Sheets dashboard (batched)
 * @version 1.0.0
 */
...
const BATCH_SIZE = 100; // Upload 100 rows at a time
...
async function appendBatch(rows, batchNum, totalBatches) {
    ...
    const cmd = `echo '${valuesJson}' | gog sheets append "${SHEET_ID}" "Sheet1!A:J" --values-json - --insert INSERT_ROWS --account ${ACCOUNT}`;
    
    try {
        await execAsync(cmd);
        console.log(`✅ Batch ${batchNum}/${totalBatches} uploaded (${rows.length} rows)`);
    } catch (error) {
...
async function main() {
    try {
        ...
        console.log(`Found ${allRows.length} total opportunities`);
        ...
        console.log(`📤 Uploading ${batches.length} batches to Google Sheets...`);
